...
class EncoderDecoder():
    ...
    def __init__(self, is_training=True, **kwargs):
        ...
        self._type = self._model_kwargs.get('data_type')
        ...
        if self._model_type == 'ed' or self._model_type == 'encoder_decoder':
            self._data = utils.load_dataset(seq_len=self._seq_len, horizon=self._horizon,
                                                    ...
                                                    r=self._verified_percentage, test_size=self._test_size,
                                                    valid_size=self._valid_size, type=self._type ,**kwargs)
        else:
        ...
        self.model = self._model_layer(is_training=is_training)
    ...
    def _get_log_dir(kwargs):
        ...
        if log_dir is None:
            ...
            data_type  = kwargs['model'].get('data_type')
            verified_percentage = kwargs['model'].get('verified_percentage')

            run_id = '%s_%s_%d_%d_%s_%d_%g/' % (
                model_type, data_type,seq_len, horizon,
    ...
    def _model_layer(self, is_training=True):
        ...
        if is_training:
            ...
            self._logger.info("Load model from: {}".format(self._log_dir))
            model.load_weights(self._log_dir + 'best_model.hdf5')
            model.compile(optimizer=self._optimizer, loss='mse', metrics=['mse'])
    ...
    def train(self):
        ...
        self.model.compile(optimizer=self._optimizer, loss='mse', metrics=['mse'])
        ...
        training_history = self.model.fit([_data['encoder_input_train'], _data['decoder_input_train']],
                                          _data['decoder_target_train'],
                                          batch_size=self._batch_size,
                                          epochs=self._epochs,
                                          callbacks=self.callbacks_list,
                                          validation_data=([_data['encoder_input_val'],
                                                            _data['decoder_input_val']],
                                                           _data['decoder_target_val']),
                                          shuffle=True,
                                          verbose=2)
    ...
    # save bm and pd to log dir
        ...
        ground_truth = scaler.inverse_transform(data_test[:_data.shape[0]])[:,-1]
        np.save(self._log_dir+'pd', predicted_data)
        np.save(self._log_dir+'gt', ground_truth)
        # save metrics to log dir
        error_list = utils.cal_error(ground_truth.flatten()[l:], predicted_data.flatten()[l:], self._log_dir, self._alg_name)
    ...
    def predict(self):
        ...
        data = np.zeros(shape=(l, self._input_dim), dtype='float32')
        data[:l, :] = data_test[:l, :]

        _data = np.zeros(shape=(l + h, self._input_dim), dtype='float32')
        _data[:l, :] = data_test[:l, :]
        # gt = data_test[l:, :]
        ...
        # ground_truth = scaler.inverse_transform(gt)[:,-1]
        return predicted_data

    def _predict(self, source):
        states_value = self.encoder_model.predict(source)
        target_seq = np.zeros((1, 1, self._output_dim))
        preds = np.zeros(shape=(self._horizon, self._output_dim), dtype='float32')
    ...
    def plot_series(self):
        ...
        plt.plot(preds[self._seq_len:], label='preds')
        plt.plot(gt[self._seq_len:], label='gt')
        plt.legend()
        plt.savefig(self._log_dir + '[result_predict]series_{}_{}.png'.format(str(self._type), str(self._rnn_layers)))
        plt.close()
